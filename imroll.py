...
class ImRoll:
	...
	async def imroll(self, ctx, *text):
		...
		await asyncio.gather(
			lolibooru_get(self, ctx, server, channel, lock),
			danbooru_get(self, ctx, server, channel, lock),
			gelbooru_get(self, ctx, server, channel, lock),
			konachan_get(self, ctx, server, channel, lock),
		)
